...
def gamma_rate(numbers: List[str]) -> str:
    m = len(numbers[0])
    counts = [Counter(number[i] for number in numbers) for i in range(m)]
...
def life_support_rating(numbers: List[str]) -> int:
    return int(oxygen_generator_rating(numbers), 2) * int(
        co2_scrubber_rating(numbers), 2
    )
...
if __name__ == "__main__":
    ...
    numbers = open(f"{cd}/input.txt").read().splitlines()
