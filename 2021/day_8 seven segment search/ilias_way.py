...
class Screen:
    def __init__(self, input_line: str) -> None:
        self.screen_digits = [
            "".join(sorted(x)) for x in input_line.split(" | ")[1].split()
        ]
        self.decoder = Decoder.parse_input(input_line)
        self.decoder.decode()

    def decode_screen(
        self, segments_mapping: Dict[str, str] = SEGMENTS_TO_DIGITS
    ) -> int:
        screen_digit_num = []
        for screen_digit in self.screen_digits:
            screen_digit_decoded = "".join(
                sorted(self.decoder.decoding_dict[x] for x in screen_digit)
            )
...
class Decoder:
    ...
    def decode(self) -> None:
        ...
        # get f
        self.decoding_dict["f"] = [k for k, v in letter_counts.items() if v == 9][0]

        # get e
        self.decoding_dict["e"] = [k for k, v in letter_counts.items() if v == 4][0]

        # get c
        self.decoding_dict["c"] = [x for x in code_one if x != self.decoding_dict["f"]][
            0
        ]

        # get b
        self.decoding_dict["b"] = [k for k, v in letter_counts.items() if v == 6][0]

        # get d
        self.decoding_dict["d"] = [
            x
            for x in code_four
            if (
                x != self.decoding_dict["b"]
                and x != self.decoding_dict["c"]
                and x != self.decoding_dict["f"]
            )
        ][0]

        # get g
        self.decoding_dict["g"] = [
            k for k in self.decoding_dict.keys() if k not in self.decoding_dict.values()
        ][0]
