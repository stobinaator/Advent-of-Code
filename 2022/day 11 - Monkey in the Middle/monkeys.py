...
class Monkey:
    ...
    def __init__(self, items, operation, divisible,truf, fals):
        ...
        self.truf = truf 
        self.fals = fals
        self.total_items_held = 0

    def get_number_of_items_holding(self):
    ...
    def add_total_items(self):
        self.total_items_held += self.get_number_of_items_holding()



def parse_monkey_objects(lines: list[list[str]]):
    monkeys = []
    for line in lines:
...
def sort_and_return_product(monkeys):
    things = [monkey.total_items_held for monkey in monkeys]
...
def get_monkey_lengths(monkeys):
    for _ in range(20): # rounds
        for monkey in monkeys:
            monkey.add_total_items()
            monkeys = monkey.use_operation(monkeys)
            monkey.items.clear() 
    return monkeys


monkey_lengths_raw = get_monkey_lengths(parse_monkey_objects(raw_lines))
lengths_raw = sort_and_return_product(monkey_lengths_raw)
assert lengths_raw == 10605
print(lengths_raw)

monkeys_lengths = get_monkey_lengths(parse_monkey_objects(lines))
p1 =  sort_and_return_product(monkeys_lengths)
print(p1)
