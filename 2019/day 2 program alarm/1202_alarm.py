...
def to_int() -> List[int]:
    with open(f'{cd}/input2.txt', 'r') as f:
        numbers = [num.split(',') for num in f]
        numbers = numbers[0]
...
def get_fours(numbers: List[int]) -> int:
    for a,b,c,d in grouper(4, numbers):
        op_code = a
        first_num = numbers[b]
        second_num = numbers[c]

        if op_code == 1:
            numbers[d] = first_num + second_num
...
def brute_force_it(numbers: List[int]) -> List[int]:
    result = list()
    for i in range(100):
        for j in range(100):
